...
class PolicyBase:
    def __init__(self):
        self._annotate_cooperative_fetch()
        self._annotate_split_axis()
        ...
        for out in output_tensors:
            ...
            if smem_bool:
                for input_tensor in input_tensors:
                    shared_tensor = self.sche_simu.cache_read(
            ...
            if reg_bool:
                for shared_tensor in smem_tensor:
                    local_tensor = self.sche_simu.cache_read(
            ...
            if binding["space"][0] is not None:
                self.sche_simu[out].bind(
                    blck_fused, te.thread_axis(binding["space"][0])
                )
            ...
            if binding["space"][1] is not None:
                self.sche_simu[out].bind(
                    thrd_fused, te.thread_axis(binding["space"][1])
                )
            ...
            if reg_tile is not None:
                for rt in reg_tensor:
                    self.sche_simu[rt].compute_at(
                        self.sche_simu[reg_tile], reduce_axis[-1]
                    )
                for st in smem_tensor:
                    self.sche_simu[st].compute_at(
                        self.sche_simu[reg_tile], reduce_axis[0]
                    )
    ...
    def _annotate_cooperative_fetch(self):
        if LatestTVM:
            self.cooperative_fetch.__annotations__ = {
                "self": None,
                "shared": None,
    ...
    def get_codegen_dict(self, rprog: rProg):
        """
    ...
    def _annotate_split_axis(self):
        if LatestTVM:
            self.split_axis.__annotations__ = {
                "self": None,
                "op": None,
                "axis": None,
                "sch": "tvm.tir.Schedule",
