...
def DataReuseScore(op, rtile, tile_tensor="output"):
    ...
    for d in range(len(dims)):
        ...
        new_dims[d] += 1
        new_rtile = rTile(
            rtile.expr, new_dims, op.SAxis(), op.RAxis(), op.GetTvmOutTensor()
        )
...
def Estimate_ActiveBlock(op, arch, rprog):
    ...
    return min(
        ...
        int(arch.max_threads_per_sm // block_size),
        arch.max_active_blocks,
    )
...
class ConstructionPolicyRT(PolicyBase):
    ...
    def DataReuseScore(self, rprog: rProg, mem_level, tile_tensor="output"):
        """
        return a list of scores on each dimension
        """
    ...
    def GetAlignedSteps(self, base_rtile: rTile, mem_level):
        """
        returns
        ...
        dbglogger.debug(f"GetAlignedSteps: {base_rtile}")
        dim = len(full_dim)
        sdim = len(full_sdim)
        steps_arch = []
    ...
    def IsComputeIntensive(self, rprog: rProg, mem_level):
        """
    ...
    def IsPeakComputeTile(self, rprog: rProg, mem_level):
        ...
        if mem_level == 0:
            ...
            if self.op.use_tc:
                num_threads *= 32
            return (
                num_threads % (self.arch.warp_size * self.arch.compute_sm_partition[1])
                == 0
            )
        # scale out
        if mem_level == -1:
    ...
    def EnlargeTile(self, last_rprog: rProg, rprog: rProg, steps, mem_level):
        key = rprog.Dump()
        ...
        def one_level_down(rprog: rProg, this_level):
    ...
    def expand_reduce_axis(self, config, reduction_axis_name, mem_level):
        ...
        if self.activeblock_db.lookup_schedule(config) != None:
            ...
            while True:
                ...
                if (
                    not eligible(self.op, self.arch, config, mem_level)
    ...
    def emit_config_without_trails(self, topk):
        ...
        while th <= self.th_cap and len(self.all_results) < topk:
            self.emit_raw_configs(padding_threshold=th)
            ...
            if len(self.top_results) == 0:
                print(
                    "Failed to find results with padding threshold {} (padding_threshold_cap)".format(
                        th
                    )
                )
            else:
                print(
                    "Found {} results with threshold {} (padding_threshold_cap)".format(
