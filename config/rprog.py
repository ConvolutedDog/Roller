...
class rProg:
    ...
    def AddTile(self, mem_level, rtile: rTile):
        self.rTiles[mem_level] = rtile

    def DeleteTile(self, mem_level):
        del self.rTiles[mem_level]

    def GetTile(self, mem_level) -> rTile:
        return self.rTiles[mem_level]

    def UpdateTile(self, new_rtile: rTile, mem_level):
    ...
    def GetParallelism(self, mem_level):
        if mem_level == 0:
            return num_tiles(
                self.full_shape[: self.spatial_dim], self.rTiles[0].SDimensions()
            )
        else:
            return num_tiles(
                self.rTiles[mem_level - 1].SDimensions(),
